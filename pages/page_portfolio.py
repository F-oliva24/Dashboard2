"""
pages/page_portfolio.py — My Portfolio.
Composizione + Top3 Watchlist + History progressiva.
"""
import json
    ...
    if is_admin:
        with st.expander("⚙️ Edit Buy Prices & Transactions", expanded=False):
            ...
            for i, (_, pos) in enumerate(positions_df.iterrows()):
                ...
                name    = str(pos.get("yf_name", pos.get("name","")))[:20]
                cur     = ISIN_MAP.get(isin,{}).get("currency", str(pos.get("currency","EUR")))
